    ...
    try:
        ...
        if extra_columns_file1:
            user_choice = common.simple_input_dialog(
                ...
                f"The following extra columns are in '{file1_path}': {', '.join(extra_columns_file1)}.\n\n"
                "Do you want to:\n"
        ...
        if not new_rows.empty:
            user_choice = common.simple_input_dialog(
                ...
                f"There are new rows in '{file2_path}' not present in '{file1_path}'.\n"
                "Do you want to add these new rows to the first file? (Type 'yes' to include or 'no' to skip):"
            )
        ...
        if not missing_rows.empty:
            user_choice = common.simple_input_dialog(
                ...
                f"The following rows are in '{file1_path}' but not in '{file2_path}':\n\n"
                f"{missing_rows[key_column].tolist()}.\n"
                "Do you want to:\n"
                "1. Keep these rows as is\n"
                "2. Set the values in these rows to NA\n\n"
                "Please type 'keep' or 'set to NA':"
            )
            if user_choice:
                if user_choice.lower() == 'set to na':
        ...
        if new_columns:
            user_choice = common.simple_input_dialog(
                ...
                f"The following new columns are in '{file2_path}': {', '.join(new_columns)}.\n"
